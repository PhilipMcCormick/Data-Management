...
ORDER BY avg_vote DESC, original_title
LIMIT 5;
...
FROM actors
WHERE
    date_of_birth LIKE '%02-29%'
AND place_of_birth LIKE '%USA%'
AND death_details IS NULL
...
FROM movies
WHERE
    LOWER(description) LIKE '%zombie%'
AND worlwide_gross_income IS NOT NULL
ORDER BY CAST(LTRIM(worlwide_gross_income, '$ ') AS BIGINT) DESC
LIMIT 5;
...
SELECT
    name,
    height AS height_cm,
    CONCAT(CAST((CAST(ROUND(height / 2.54) AS INT) / 12) AS VARCHAR), ' feet ',
           CAST(ROUND((height / 2.54) - ((CAST(ROUND(height / 2.54) AS INT) / 12) * 12)) AS VARCHAR), ' inches') AS height_ft
FROM actors
WHERE
    height IS NOT NULL
    AND height <= 300
...
SELECT
    ...
    usa_gross_income,
    worlwide_gross_income,
    ROUND(100.0 * CAST(LTRIM(usa_gross_income, '$ ') AS NUMERIC)/
    CAST(LTRIM(worlwide_gross_income, '$ ') AS NUMERIC), 1) AS pct_gross_income_usa
FROM movies
WHERE
