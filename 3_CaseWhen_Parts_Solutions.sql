...
SELECT
    CASE WHEN avg_vote >= 9.0 THEN 'Great'
         WHEN avg_vote >= 8.0 THEN 'Good'
         WHEN avg_vote >= 7.0 THEN 'Decent'
         WHEN avg_vote >= 6.0 THEN 'Questionable'
 ...
 */
SELECT
    COUNT(1) AS num_2008_movies_above_average
FROM movies
WHERE
    year = 2008
    AND votes >= 10000
    AND avg_vote > (
        SELECT AVG(avg_vote) AS avg_rating_2008
        FROM movies
        WHERE votes >= 10000
          AND year = 2008
    )
...
SELECT
    CASE WHEN DATE_PART('month', DATE(date_published)) IN (12,1,2) THEN 'Winter'
         WHEN DATE_PART('month', DATE(date_published)) IN (3,4,5) THEN 'Spring'
         WHEN DATE_PART('month', DATE(date_published)) IN (6,7,8) THEN 'Summer'
         WHEN DATE_PART('month', DATE(date_published)) IN (9,10,11) THEN 'Fall'
         END AS season,
    COUNT(1) AS num_movies,
...
SELECT
    CONCAT(LEFT(CAST(DATE_PART('year', DATE(date_of_birth)) AS VARCHAR), 3), '0s') AS birth_decade,
    COUNT(CASE WHEN place_of_birth LIKE '%California%' THEN imdb_name_id ELSE NULL END ) AS count_california_born,
    COUNT(1) AS count_total,
    ROUND(COUNT(CASE WHEN place_of_birth LIKE '%California%' THEN imdb_name_id ELSE NULL END ) * 100.0 / COUNT(1), 1) AS pct_california_born
FROM actors
WHERE
 ...
 */
SELECT
    CASE DATE_PART('dow', DATE(date_published))
        WHEN 0 THEN 'Sunday'
        WHEN 1 THEN 'Monday'
        WHEN 2 THEN 'Tuesday'
        ...
        ELSE 'Other' END AS day_of_week,
    COUNT(1) AS num_movies
FROM movies
WHERE
    date_published LIKE '____-__-__'
GROUP BY 1
ORDER BY 2 DESC;
