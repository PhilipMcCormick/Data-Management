...
FROM ratings
INNER JOIN movies m
    ON ratings.imdb_title_id = m.imdb_title_id
...
WHERE
    a.name IN ('Tom Hanks', 'Morgan Freeman')
...
SELECT
    original_title,
    (females_18age_avg_vote + males_18age_avg_vote)/2 AS avg_vote_18_30,
    (females_45age_avg_vote + males_45age_avg_vote)/2 AS avg_vote_45plus,
    (females_45age_avg_vote + males_45age_avg_vote)/2 - (females_18age_avg_vote + males_18age_avg_vote)/2  AS delta
FROM ratings
INNER JOIN movies m
    ON ratings.imdb_title_id = m.imdb_title_id
WHERE
    males_18age_votes >= 10000
AND females_18age_votes >= 10000
AND females_45age_votes >= 10000
AND males_45age_votes >= 10000
ORDER BY 4 DESC, 1
LIMIT 5;
...
SELECT
    CASE WHEN LOWER(m.description) LIKE '%basketball%' THEN 'Basketball Movie'
         ELSE 'Non-Basketball Movie' END AS movie_type,
    COUNT(DISTINCT m.imdb_title_id) AS num_movies,
    COUNT(DISTINCT a.imdb_name_id) AS num_actors,
...
FROM ratings
INNER JOIN title_principals tp ON ratings.imdb_title_id = tp.imdb_title_id
INNER JOIN actors a on tp.imdb_name_id = a.imdb_name_id
INNER JOIN movies m on ratings.imdb_title_id = m.imdb_title_id
WHERE
    tp.category = 'actress'
...
SELECT
    name,
    COUNT(CASE WHEN avg_vote >= 8 THEN m.imdb_title_id ELSE NULL END) AS num_8plus_movies,
    COUNT(1) AS num_total_movies,
    ROUND(COUNT(CASE WHEN avg_vote >= 8 THEN m.imdb_title_id ELSE NULL END) * 100.0 / COUNT(1), 1) AS pct_8plus_movies
...
SELECT
    m.original_title,
    COUNT(1) AS num_child_actors
FROM title_principals tp
INNER JOIN actors a on tp.imdb_name_id = a.imdb_name_id
INNER JOIN movies m on tp.imdb_title_id = m.imdb_title_id
WHERE
    a.date_of_birth LIKE '____-__-__'
    AND m.date_published LIKE '____-__-__'
    AND tp.category IN ('actor', 'actress')
    AND AGE(CAST(date_published AS TIMESTAMP), CAST(date_of_birth AS TIMESTAMP)) < INTERVAL '18 years'
    AND m.votes >= 10000
GROUP BY 1
ORDER BY 2 DESC, 1
LIMIT 10;
